...
const { isWhitelisted, isRevoked, isBanned, createSession } = require('../models/userModel');
...
function login(req, res) {
  ...
  if (!validateU00Format(u00)) {
    return res.status(400).json({ error: 'Formato U00 inválido' });
  }
  if (!isWhitelisted(u00)) {
    return res.status(403).json({ error: 'Código U00 no aprobado' });
  }
  if (isRevoked(u00)) {
