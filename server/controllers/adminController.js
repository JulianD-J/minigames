...
function emitGameState(req) {
  const io = req.app.get('io');
  if (io) io.to('lobby').emit('game:state', globalState);
}
...
function freezePlayer(req, res) {
  ...
  const updated = updatePlayer('lobby', token, { status: frozen ? 'frozen' : 'active' });
  const io = req.app.get('io');
  if (io && updated) io.to('lobby').emit('player:state', updated);
...
function gameControl(req, res) {
  ...
  if (action === 'restart') {
    ...
    const io = req.app.get('io');
    if (io) io.to('lobby').emit('race:restart', { at: Date.now() });
  }
